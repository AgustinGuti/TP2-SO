...
void setFontSize(uint8_t size, uint8_t rewrite){
	if (size > 0 && size <= MAX_FONT_SIZE){	//Size can't be 0 or greater than 4
		fontSize = size;
	}
	if(rewrite){
...
void drawRect(pxlCoord coord, uint32_t color, uint16_t width, uint16_t height){
	if (coord.x + width > getScreenWidth() || coord.y + height > getScreenHeight()){		//Check screen bounds
		return;
	}
    uint8_t bpp = getScreenBpp();
    uint16_t pitch = getScreenPitch();
...
void putChar(uint32_t color, uint16_t id){
	...
	if (!reprinting && id != BACKSPACE){				//Don't insert backspaces
		characterBuffer[lastChar++] = id;
		if (lastChar == MAX_CHAR_BUFFER){
			lastChar = 0;
		}
		if (lastChar == firstChar){		//Oldest characters are deleted
			firstChar++;
		}
	...
	switch (id){
		...
		case BACKSPACE:
			...
			break;
		default:
			if (charsToEndOfLine == 0 && currentCharOffsetToStart != 0){		//Check if another character fits
				newline();
			}
			for (int i = 0; i < CHAR_HEIGHT*fontSize; i++){
				for (int j = 0; j < charPixelWidth; j++){
					for (int k = 0; k < bytePerPixel; k++){		//We are in 24 bit mode, but this should be valid for different bpp
